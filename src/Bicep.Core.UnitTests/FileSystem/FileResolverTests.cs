...
namespace Bicep.Core.UnitTests.FileSystem
{
    ...
    public class FileResolverTests
    {
        ...
        public void TryReadWithLimit_should_return_expected_results()
        {
            ...
            fileResolver.TryRead(tempFileUri, out var fileContents, out var failureMessage, Encoding.Default, 6).Should().BeFalse();
            fileContents.Should().BeNull();
            failureMessage.Should().NotBeNull();
            ...
            fileResolver.TryRead(tempFileUri, out fileContents, out failureMessage, Encoding.Default, 6).Should().BeFalse();
            fileContents.Should().BeNull();
            failureMessage.Should().NotBeNull();
        }
